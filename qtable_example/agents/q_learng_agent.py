...
class QLearningAgent(BaseAgent):
    ...
    def act(self, state: tuple, valid_moves: list[T]) -> T:
        ...
        if np.random.rand() < self.exploration_rate:
            ...
            mapped_q_values = {action.value: q_values[action.value] for action in valid_moves}
            q_values = np.array([mapped_q_values[action.value] for action in valid_moves])
            max_q_value = np.max(q_values)
            best_actions = [action for action in valid_moves if mapped_q_values[action.value] == max_q_value]
